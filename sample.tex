...
\textbf{$\beta$}: This is defined as $\beta = \frac{1}{k_B T}$, where $k_B$ is the Boltzmann constant and $T$ is the temperature of the system. $\beta$ essentially measures how sensitive the system is to energy changes at a given temperature.
...
\noindent Procedure of Monte Carlo Move Algorithm\\


\begin{enumerate}
    \item Choose a bead at random.
    \item Propose a small displacement:
    \[
    \vec{r}_i' = \vec{r}_i + \Delta \vec{r}, \quad \Delta \vec{r} \in [-\delta, \delta]
    \]
    \item Compute energy change $\Delta E$:
    \begin{itemize}
        \item Recalculate FENE bonds for affected neighbors.
        \item Recalculate WCA interactions for all non-bonded beads within cutoff.
    \end{itemize}
    \item Accept or reject using the Metropolis criterion:
    \[
