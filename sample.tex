...
particles are immature virions \cite{goldsmith2003monkeypox} \cite{}}
\end{figure}
...
\end{comment}
          ...
          \indent\indent  WCA potential, a purely repulsive form of the Lennard-Jones (LJ) potential for excluded volume (non-bonded interactions).
...
\begin{comment}
the assumption is that all particles have the same size, which is not true in reality. r_ij^2 + 1e-6 < 1 is a confusion statement. Since all particles have the same size in the simulation r_ij^2 < sigma where sigma is the diameter of a particle and is the length unit of the simulation. 1-e6 is for numerical purpose and should not be in Eq. 7
\end{comment}
...
\begin{comment}
\textcolor{red}{\textbf{Here, the assumption is that all particles have the same size, which is not true in reality. less than 1 is a confusion statement. Since all particles have the same size in the simulation sigma where sigma is the diameter of a particle and is the length unit of the simulation. 1-e6 is for numerical purpose and should not be in Eq7}}
\end{comment}
...
\noindent \textbf{Hypothesis 2:}\\
If a genome-like rod and crowder particles are confined inside an initially cubic, flexible membrane/box, then after equilibrating the internal packing in NVT and subsequently allowing box-shape fluctuations in NPT, the system will spontaneously break cubic symmetry and evolve toward an elongated cuboid (rectangular-prism) equilibrium shape.
...
\noindent Work 1: Genome Geometry–Driven Membrane Deformation. Establish how genome length, stiffness, and geometry control membrane shape under confinement.
 
(1)Develop a coarse-grained bead–spring genome model confined within a flexible Helfrich–Canham membrane.

(2)Systematically vary:

\indent\indent (i)genome length (short RNA vs long DNA)

\indent\indent (ii)genome stiffness (flexible vs rod-like)

\indent\indent (iii)membrane size and bending rigidity

\noindent Work 2: This work will examine how internal pressure affects the size and geometry of a virus, and how weak attractions between the genome and the surrounding cellular environment influence host cell membrane after host-cell entry. A simple coarse-grained model with Lennard-Jones attractions will be used, in which genome length, stiffness, and interaction strength are varied to observe how virus shape and genome position respond.

\noindent Work 3: Shape Symmetry Breaking Explain why some viruses (e.g., Monkeypox) transition from spherical to elongated shapes,



\begin{comment}
\textcolor{red}{\textbf{You may need to remove project timeline and add the specific 3 pieces work you plan to do. Each one will eventually lead to a publication and should be very brief.}}
\end{comment}
